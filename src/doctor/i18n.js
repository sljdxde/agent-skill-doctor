...
const en = {
  ...
  'guide.zombie.steps': 'Fix steps:\n1. Check the recorded config references and activity evidence\n2. Test if the skill still works properly\n3. If it is intentionally unused:\n   - Archive or disable the skill first\n   - Update related configurations\n   - Delete the skill directory only after review\n4. If it is still needed:\n   - Update the skill description\n   - Test compatibility with current agent\n   - Add trigger condition documentation',
  'guide.zombie.prompt': 'Please evaluate the skill at "%s". Check recorded config references and activity evidence. If it is intentionally unused, archive or disable it first; delete only after review. If needed, update its description and verify compatibility.',
...
const zh = {
  ...
  'guide.zombie.prompt': '请评估技能 "%s"。检查已记录的配置引用和活动证据。如确认是有意闲置，先归档或禁用，复核后再删除；如仍需使用，则更新描述并验证兼容性。',
