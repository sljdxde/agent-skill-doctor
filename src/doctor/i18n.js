...
const en = {
  ...
  'tooltip.zombie.text': 'Zombie score (0.0-1.0) based on 6 weighted factors. Score >= 0.4 triggers a finding. Protected: pinned/keep/core/system tags (score 0), official sources like anthropics/openai (score 0), plugin sources like obra/superpowers (score *0.5). Third-party sources get score *0.75. Findings also include usage classification, confidence, and detected config references.',
  ...
  'guide.risk.prompt': 'Please review the skill at "%s" and propose safety guardrails. Show the evidence and a minimal diff first; do not edit anything until explicitly confirmed.',
  ...
  'guide.zombie.steps': 'Review steps:\n1. Check whether the skill is referenced in any project config\n2. Verify activity evidence and test whether it still works\n3. Classify it as keep, monitor, or review-needed\n4. If a change is confirmed, prepare a reversible proposal and update related configurations only after approval',
  'guide.zombie.prompt': 'Please review the skill at "%s". Verify references and activity evidence, classify it as keep/monitor/review-needed, and propose only reversible changes. Do not remove or edit anything without explicit confirmation.',
  ...
  'guide.duplicate.steps': 'Review steps:\n1. Compare content, source, references, and installation scope\n2. Decide whether the candidates are intentionally installed\n3. Mark a canonical candidate only when evidence is high confidence\n4. Prepare a reversible consolidation proposal; change presets or files only after approval',
  'guide.duplicate.prompt': 'Please review the duplicate skill group. Verify evidence and scope, mark only high-confidence consolidation candidates, and propose a reversible plan. Do not remove, disable, or edit anything without explicit confirmation.',
  ...
  'guide.conflict.steps': 'Review steps:\n1. Confirm the instructions apply to the same agent, scope, and workflow\n2. Compare the evidence and determine whether the conflict is real\n3. Propose wording or scope boundaries as a minimal diff\n4. Preserve both skills unless the user explicitly approves a change',
  'guide.conflict.prompt': 'Please review the conflict between skills at "%s". Verify scope and evidence, then propose minimal wording or scope changes. Do not remove or edit anything without explicit confirmation.',
  ...
  'guide.version_drift.steps': 'Review steps:\n1. Confirm the installations share the same declared upstream identity\n2. Compare refs, content hashes, compatibility, and active references\n3. Identify the desired version only with supporting evidence\n4. Propose a pinned, reversible sync plan; update or reinstall only after approval',
  'guide.version_drift.prompt': 'Please review installations of skill "%s" for confirmed version drift. Verify the shared upstream identity and compatibility, then propose a reversible sync plan. Do not update, reinstall, or consolidate anything without explicit confirmation.',
  ...
  'guide.freshness.prompt': 'Please review whether the skill at "%s" is outdated. Verify its source and pin, then propose a reversible update plan. Do not add, update, reinstall, or edit anything without explicit confirmation.',
  ...
  'guide.scan_warning.prompt': 'Please review the structural issues with the skill at "%s". Verify the loader contract and propose a minimal reversible diff; do not create, replace, or edit files without explicit confirmation.',
...
const zh = {
  ...
  'tooltip.zombie.text': '僵尸分数（0.0-1.0）基于 6 个加权因子。分数 >= 0.4 触发发现。受保护：pinned/keep/core/system 标签（分数 0）、官方来源如 anthropics/openai（分数 0）、插件来源如 obra/superpowers（分数 *0.5）。第三方来源分数 *0.75。结果还会显示使用分类、证据置信度和检测到的配置引用。',
  ...
  'guide.risk.prompt': '请复核技能 "%s" 并提出安全防护方案。先展示证据和最小差异，未明确确认前不得编辑任何内容。',
  ...
  'guide.zombie.steps': '复核步骤：\n1. 检查技能是否在任何项目配置中被引用\n2. 核验活跃证据并测试技能是否还能正常工作\n3. 将其分类为保留、观察或需要复核\n4. 只有获得确认后，才能提出可回退的变更并更新相关配置',
  'guide.zombie.prompt': '请复核技能 "%s"。核验引用和活跃证据，将其分类为保留/观察/需要复核，并且只提出可回退的方案。未明确确认前不得移除或编辑任何内容。',
  ...
  'guide.duplicate.steps': '复核步骤：\n1. 比较内容、来源、引用和安装范围\n2. 判断这些候选项是否是有意安装的\n3. 只有证据充分时才标记规范候选\n4. 先输出可回退的合并方案，获得确认后才能修改预设或文件',
  'guide.duplicate.prompt': '请复核重复技能组。核验证据和作用范围，只标记高置信度的合并候选，并提出可回退方案。未明确确认前不得移除、禁用或编辑任何内容。',
  ...
  'guide.conflict.steps': '复核步骤：\n1. 确认这些指令确实作用于同一个 Agent、范围和工作流\n2. 比较证据，判断冲突是否真实存在\n3. 以最小差异提出措辞或作用范围调整\n4. 除非用户明确批准，否则保留全部 Skill',
  'guide.conflict.prompt': '请复核技能 "%s" 之间的冲突。核验作用范围和证据后，提出最小化的措辞或范围调整。未明确确认前不得移除或编辑任何内容。',
  ...
  'guide.version_drift.steps': '复核步骤：\n1. 确认这些安装声明了相同的上游身份\n2. 比较 ref、内容哈希、兼容性和当前引用\n3. 只有有证据支持时才确定期望版本\n4. 提出锁定版本的可回退同步方案，获得确认后才能更新或重装',
  'guide.version_drift.prompt': '请复核技能 "%s" 的已确认版本漂移。核验共同上游身份和兼容性后，提出可回退的同步方案。未明确确认前不得更新、重装或合并任何内容。',
  ...
  'guide.freshness.prompt': '请复核技能 "%s" 是否需要更新。核对来源和版本锁定后，提出可回退的更新方案。未明确确认前不得添加、更新、重装或编辑任何内容。',
  ...
  'guide.scan_warning.prompt': '请复核技能 "%s" 的结构问题。先核实加载约定并提出最小可回退差异，未明确确认前不得创建、替换或编辑文件。',
